import telebot, json, os, time, threading, traceback, html
...
# CHANGE ONLY THESE 2
# =====================
BOT_TOKEN = "8963887399:AAEEeYRAaAfeXhBK4ef7hmbs748U4S1nVFo"
OWNER_ID = 7142950609  # apna Telegram numeric ID

bot = telebot.TeleBot(BOT_TOKEN)  # parse_mode removed = reward HTML error fixed
...
# OPEN ADMIN PANEL
# =====================
@bot.callback_query_handler(func=lambda c: c.data == "open_admin_panel")
...
def start_keyboard(user_id=None):
    ...
    if user_id and is_admin(user_id):
        kb.add(types.InlineKeyboardButton("👑 ADMIN PANEL", callback_data="open_admin_panel"))
...
def send_welcome(message):
    ...
    try:
        ...
        if photos.total_count > 0:
            ...
            bot.send_photo(message.chat.id, file_id, caption=text, reply_markup=start_keyboard(user.id))
        else:
            bot.send_message(message.chat.id, text, reply_markup=start_keyboard(user.id))
    except:
        bot.send_message(message.chat.id, text, reply_markup=start_keyboard(user.id))
...
def verify_join(call):
    ...
    if not ok:
        ...
        return bot.send_message(call.message.chat.id, txt, reply_markup=start_keyboard(call.from_user.id))
...
def claim_reward(call):
    ...
    return bot.send_message(call.message.chat.id, "❌ Refer system OFF hai.")
...
def state_handler(message):
    ...
    if state == "add_channel":
        try:
            ...
            return bot.reply_to(message, "❌ Format: NAME | @USERNAME | LINK")
    ...
    if state == "add_reward":
        ...
        if message.content_type == "text":
            ...
            db["rewards"][rid] = {"type": "photo", "content": message.photo[-1].file_id, "caption": message.caption or ""}
        elif message.content_type == "video":
            db["rewards"][rid] = {"type": "video", "content": message.video.file_id, "caption": message.caption or ""}
        elif message.content_type == "document":
            filename = message.document.file_name or ""
            rtype = "apk" if filename.lower().endswith(".apk") else "document"
            db["rewards"][rid] = {"type": rtype, "content": message.document.file_id, "caption": message.caption or ""}
    ...
    if state == "add_ads":
        ...
        if message.content_type == "text":
            ...
            db["ads"][aid] = {"type": "photo", "content": message.photo[-1].file_id, "caption": message.caption or ""}
        elif message.content_type == "video":
            db["ads"][aid] = {"type": "video", "content": message.video.file_id, "caption": message.caption or ""}
        elif message.content_type == "document":
            db["ads"][aid] = {"type": "document", "content": message.document.file_id, "caption": message.caption or ""}
    ...
    if state == "text_broadcast":
        done, fail = broadcast_item({"type": "text", "content": message.text})
        clear_state(uid)
    ...
    if state == "button_broadcast":
        try:
            ...
            return bot.reply_to(message, "❌ Format: Message | Button Name | Link")
